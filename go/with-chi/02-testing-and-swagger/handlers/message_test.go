...
func TestGetLatestMessages(t *testing.T) {
	...
	t.Run("Get Latest 10 Messages in Reverse Order", func(t *testing.T) {
		...
		// Check if messages are in reverse order
		if messages[0].Text != "Message 14" {
			t.Errorf("Expected first message to be 'Message 14', got %v", messages[0].Text)
		}
		if messages[9].Text != "Message 5" {
			t.Errorf("Expected last message to be 'Message 5', got %v", messages[9].Text)
		}
	})

	t.Run("Get All Messages in Reverse Order When Less Than 10", func(t *testing.T) {
		// Reset handler with fewer messages
		handler = setupTestHandler() // Only 2 messages initially
		...
		// Check reverse order
		if messages[0].From != "Lisa" {
			t.Errorf("Expected first message from 'Lisa', got %v", messages[0].From)
		}
		if messages[1].From != "Bart" {
			t.Errorf("Expected second message from 'Bart', got %v", messages[1].From)
		}
	})
}
